...
const PaymentTab = ({ paymentOptions, getSystemStatus, onConnect, onDisconnect, customIdByName = {} }) => {
  return (
    <div className="space-y-5">
      {paymentOptions.map((payOpt) => {
        const isCustom = Boolean(customIdByName[payOpt]);
        const status = getSystemStatus(payOpt);
        ...
        return (
          ...
          >
            <div className="flex items-center gap-5 sm:gap-6 mb-4 sm:mb-0">
              ...
              <div>
                <span className="text-lg sm:text-xl font-bold text-[#0D2C3D] block">{payOpt} Gateway</span>
                {isConnected && (
                  <span className="text-sm text-[#1F5E88] font-bold block">Status: Integrasi API Secure Active</span>
                )}
              </div>
            </div>
