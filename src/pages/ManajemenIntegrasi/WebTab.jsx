...
const WebTab = ({ webOptions, getSystemStatus, onConnect, onDisconnect, customIdByName = {} }) => {
  return (
    <div className="space-y-5">
      {webOptions.map((webOpt) => {
        const isCustom = Boolean(customIdByName[webOpt]);
        const status = getSystemStatus(webOpt);
        ...
        return (
          ...
          >
            <div className="flex items-center gap-5 sm:gap-6 mb-4 sm:mb-0">
              ...
              <div>
                <span className="text-lg sm:text-xl font-bold text-[#0D2C3D] block">{webOpt} Integration</span>
                {isConnected && (
                  <span className="text-sm text-[#1F5E88] font-bold block">Status: Pemantauan Integritas Web Aktif</span>
                )}
              </div>
            </div>
