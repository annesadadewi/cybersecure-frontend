...
const Header = ({ activeMenu, user, onToggleSidebar, unreadCount = 0 }) => {
  const isFeaturesPage = activeMenu === 'Pelajari Fitur';
  const isIntegrationsPage = activeMenu === 'Manajemen Integrasi';
  const isTallHeader = isIntegrationsPage;

  return (
    <header 
      className={`${isTallHeader ? 'h-24 lg:h-28' : 'h-20 lg:h-24'} px-4 sm:px-6 lg:px-10 flex justify-between items-center backdrop-blur-xl z-10 border-b border-white/10 shadow-lg shrink-0`} 
