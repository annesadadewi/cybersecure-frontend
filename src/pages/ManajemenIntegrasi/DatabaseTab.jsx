...
const DatabaseTab = ({ databaseOptions, getSystemStatus, onConnect, onDisconnect, customIdByName = {} }) => {
  return (
    <div className="space-y-5">
      {databaseOptions.map((dbOpt) => {
        const isCustom = Boolean(customIdByName[dbOpt]);
        const status = getSystemStatus(dbOpt);
        ...
        return (
          ...
          >
            <div className="flex items-center gap-5 sm:gap-6 mb-4 sm:mb-0">
              ...
              <div>
                <span className="text-lg sm:text-xl font-bold text-[#0D2C3D] block">{dbOpt} Database</span>
                {isConnected && (
                  <span className="text-sm text-[#1F5E88] font-bold block font-mono">Host: 127.0.0.1 | Port: {dbOpt === 'PostgreSQL' ? '5432' : '3306'}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {isConnected ? (
                <button 
                  onClick={() => onDisconnect({ id: isCustom ? customIdByName[dbOpt] : `db-${dbOpt}`, name: `${dbOpt} Database`, type: 'Database', customTabId: isCustom ? 'database' : undefined })}
                ...
                <button 
                  onClick={() => onConnect({ id: isCustom ? customIdByName[dbOpt] : `db-${dbOpt}`, name: `${dbOpt} Database`, type: 'Database', customTabId: isCustom ? 'database' : undefined })}
