...
import { RefreshCw, KeyRound, X } from 'lucide-react';

const MarketplaceTab = ({ loading, mergedMarketplaces, onConnect, onDisconnect, onResetPassword }) => {
  return (
    <div className="space-y-5">
      {loading && mergedMarketplaces.length === 0 ? (
          ...
          return (
            ...
            >
              <div className="flex items-center gap-5 sm:gap-6 mb-4 sm:mb-0">
                <div className="w-11 h-11 sm:w-12 sm:h-12 bg-[#538CB4] rounded-xl flex items-center justify-center text-white text-xl sm:text-2xl font-bold shadow-inner shrink-0">
                  {item.initial}
                </div>
                <div className="min-w-0">
                  <span className="text-lg sm:text-xl font-bold text-[#0D2C3D] block truncate">{item.name}</span>
                  {isConnected && (
                    <span className="text-sm text-[#1F5E88] font-bold block truncate">{item.email}</span>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className={`flex items-center gap-2 px-3.5 py-1.5 rounded-full border ${
                  ...
                  <span className="text-[11px] font-black uppercase tracking-wider">{isConnected ? 'CONNECTED' : 'NOT CONNECTED'}</span>
                </div>

                {isConnected ? (
                  <button 
                    onClick={() => onDisconnect({ id: item.id, dbId: item.dbId, name: item.name, type: 'Marketplace' })}
                  ...
                  <button 
                    onClick={() => onConnect({ id: item.id, name: item.name, type: 'Marketplace' })}
